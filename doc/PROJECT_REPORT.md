...
3. [系统架构与实现](#3-系统架构与实现)
4. [误差分析](#4-误差分析)
5. [与参考程序的对比分析](#5-与参考程序的对比分析)
6. [创新点与优化策略](#6-创新点与优化策略)
7. [使用说明](#7-使用说明)
8. [结论与展望](#8-结论与展望)
9. [参考文献](#9-参考文献)
...
| 参数 | 本项目 | RPA 软件 | 相对误差 |
|------|--------|----------|----------|
| 燃烧温度 T_c | 3917 K | 3589 K | +9.1% |
| 比热比 γ | 1.141 | 1.196 | -4.6% |
| 等熵指数 γ_s | 1.127 | 1.141 | -1.2% |
| 特征速度 c* | 2386 m/s | 2360 m/s | +1.1% |
| 真空比冲 I_sp,vac | 452 s | 452 s | ~0% |

**分析**：
- 燃烧温度偏高 ~9%，原因是本项目计算的是**理论绝热火焰温度**
- RPA 和 NASA CEA 的结果更接近实际值，可能考虑了离解再复合损失
- 比冲等性能参数误差在工程允许范围内
...
早期开发的参考程序 (`references/热力计算-燃烧室+喷管-待完善/`) 计算 RS-25 发动机燃烧温度为 **3471K (3198°C)**，而本项目计算结果为 **~3916K (3643°C)**，差异达 445K。

### 5.2 根本原因：7 系数多项式高温失效

参考程序使用 **7 系数 Shomate 多项式**：

$$
\frac{H}{R} = a_1 T + \frac{a_2 T^2}{2} + \frac{a_3 T^3}{3} + \frac{a_4 T^4}{4} + \frac{a_5 T^5}{5} + a_6
$$

**问题**：该多项式是针对 **300-1000K** 温度区间拟合的，在高温下外推会严重失效。

### 5.3 详细计算验证

以 H₂O 为例，使用参考程序的系数在 T=3471K 时计算焓：
...
**NASA 9 系数正确结果**：$H_{H_2O}(3471K) = -87.1$ kJ/mol

### 5.4 物理意义分析

H₂O 的标准生成焓为 **-241.8 kJ/mol**（298K），表示从 H₂ + ½O₂ 生成 H₂O 是**强放热反应**。

即使加热到 4000K，吸热约 186 kJ/mol：

$$
H(4000K) \approx -241.8 + 186 = -55.8 \text{ kJ/mol}
$$

**仍然是负值！** 7 系数多项式算出的 +73.3 kJ/mol 是**物理上荒谬的**。

### 5.5 H₂O 焓对比表

| 温度 (K) | 7 系数 (kJ/mol) | NASA 9 (kJ/mol) | 误差 |
|----------|-----------------|-----------------|------|
| 298 | -241.8 | -241.8 | ~0 |
| 1000 | -215.8 | -215.8 | ~0 |
| 2000 | -166.1 | -179.5 | 13.4 |
| **3000** | **-53.9** | **-114.2** | **60.3** ⚠️ |
| **3471** | **+73.3** | **-87.1** | **160.4** ❌ |
| **4000** | **+343.5** | **-55.6** | **399.1** ❌❌ |

### 5.6 结论

| 程序 | 燃烧温度 | 评价 |
|------|----------|------|
| 参考程序 (7系数) | 3471K | ❌ **错误**（多项式外推失效） |
| 本项目 (NASA 9) | ~3916K | ✅ **正确**（理论绝热火焰温度） |
| 维基百科 | ~3573K | 📚 考虑热损失的实际值 |
...
- [7 系数多项式问题分析](../references/热力计算-燃烧室+喷管-待完善/7系数多项式问题分析.md)
